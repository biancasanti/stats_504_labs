...
data$mass_trans_spend_right <- as.factor(data$mass_trans_spend_right)

ggplot(data %>%
         group_by(polviews) %>%
         count(mass_trans_spend_right) %>%
         mutate(proportion = n / sum(n)), 
       aes(x = polviews, y = proportion, fill = mass_trans_spend_right)) +
  geom_col() 
...
intercept_only_model <- glm(
  mass_trans_spend_right ~ 1, data = data
  ) 

# NOTE TO SELF: I think I need to redo this so that mass_trans_spend_right is not a 
# factor in the og data df
