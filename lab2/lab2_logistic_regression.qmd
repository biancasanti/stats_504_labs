...
mass_spend_long <- pivot_longer(mass_spend_summary, cols = proportion)
...
ggplot(mass_spend_summary, 
       aes(x = "", y = proportion, fill = mass_trans_spend_right)) +
  geom_col() 
...
ggplot(polviews_summary, 
       aes(x = "", y = proportion, fill = polviews)) +
  geom_col() 
...
ggplot(data %>%
         ...
         count(mass_trans_spend_right) %>%
         mutate(proportion = n / sum(n)), 
       aes(x = polviews, y = proportion, fill = factor(mass_trans_spend_right))) +
  geom_col() 

```

ANSWER: People with more conservative political views are more likely to think that the amount of spending on mass transportation is right.
...
intercept_only_model <- glm(
  mass_trans_spend_right ~ 1, data = data, family = binomial(link = "logit")
  ) 
...
```{r}
b0 <- intercept_only_model$coefficients[1] # get coef
...
Interpretation: In this dataset, there is generally a `r b0_transformed` probability of thinking the amount of spending on mass transportation is correct; the mean probability falls within the interval `r p_lower` - `r p_upper` 95% of the time.
...
data <- data %>% 
  rename(age_og = age, 
         sei10_og = sei10) %>%
  mutate(age_centered = age_og - mean(age_og),
         sei10_centered = sei10_og - mean(sei10_og)) %>%
  rename(age = age_centered, 
         sei10 = sei10_centered)
...
bage <- coef(m1)["age"]
se_age <- summary(m1)$coefficients["age", "Std. Error"] # get SE from model
...
A one unit increase in age is associated with a decrease in the log-odds of being satisfied with spending on mass transportation by 0.00617 (95% CI -0.0106, -0.00169), holding all other variables constant. The odds ratio is 0.994 (95% CI 0.989, 0.998), which confirms the negative relationship implied by the log-odds coefficient. Specifically, for each additional unit of age, the odds of being satisfied with mass transportation spending decrease by a factor of about 0.994, or approximately 99.4% per unit increase in age, holding other factors constant.
...
```{r}
bses <- coef(m1)["sei10"]
se_ses <- summary(m1)$coefficients["sei10", "Std. Error"] # get SE for sei10
...
A one unit increase in SES index is associated with a decrease in the log-odds of being satisfied with spending on mass transportation by -0.00623 units (95% CI -0.00948, -0.00297), holding all other variables constant. The odds ratio is 0.994 (95% CI 0.991, 0.997), which confirms the negative relationship implied by the log-odds coefficient. Specifically, for each additional unit of SES index, the odds of being satisfied with mass transportation spending change by a factor of about 0.994, or approximately 99.4% per unit increase in SES index, holding other factors constant.
...
-   The marginal effect for being male compared to female (NOTE: changed because of difference in level order) is -0.0007388 (95% CI -0.1015743, -0.0245632). This indicates that females are, on average, about 0.07 percentage points more likely than males to be satisfied with mass transportation spending, holding other factors constant.
...
-   We next use the model to produce visualizations:
    ...
    -   Given the code below, interpret what is being plotted:
        ...
        -   pol_plot : Model-predicted average probability of thinking that the amount of spending on mass transportation is correct for each political view category, with 95% confidence intervals.

        -   sex_plot : Model-predicted average probability of thinking that the amount of spending on mass transportation is correct for both sexes, with 95% confidence intervals.

        -   ses_plot: Model-predicted average probabilities for each possible value of SES, with shaded areas showing the 95% confidence interval.
...
```{r}
plot(binned_residuals(fit2))
```

::: callout-note
Answer: Generally yes (residuals appear to be pretty normally distributed), although not enough residuals are inside the error bounds.
:::
...
> R2 interpretation: The model fit is pretty bad -- only around 0.9% from 0 to 100%. 
>
...
::: callout-note
SES residuals seem to be fairly non-linear.
:::
...
fit2_improved <- glm(
  mass_trans_spend_right ~ age + sex + I(sei10^2), 
  data = data, 
  family = binomial(link = "logit")
)
...
# NOTE: test_likelihoodratio() isn't working because the models are not nested; trying test_vuong() instead as suggested by error message
test_vuong(fit2, fit2_improved) %>% kable()

# Check binned residuals again
binned_residuals(fit2_improved, term = "sei10") %>% plot(show_dots = TRUE)
```

::: callout-note
Did adding the squared term improve the model? 

No, not really; improvement was non-significant (p = 0.28). 
:::
...
-   Conservatives have 2.17 to 2.7 times the odds of supporting mass transit spending compared to extremely liberal and liberal respondents

-   Extreme liberals have around 0.4 times the odds of supporting spending compared to conservatives, moderates, and slightly conservative respondents

-   Extremely conservative respondents have 2.08 to 3.07 times the odds of supporting mass spending compared to liberals and slightly liberal respondents

-   Liberals have around 0.5 times the odds of supporting spending compared to moderates and slightly conservative respondents

## Conclusion

Women, people with lower SES, and more conservative people are more likely to be satisfied with the amount of mass transit spending, with political views explaining most of the variance, based on the model summary below, and the three figures.
