---
title: "Lab: Intro to Multilevel Models"
...
### ANSWER: 
All the lines are the same for each type of toy, which doesn't really match the actual values for each toy. 
...
```{r}
#DID YOUR ANSWER DIFFER FROM THE SOLUTION? IF SO, HOW?


```

### ANSWER: 
My answer didn't really differ from the solution -- I did additionally that the predicted lines don't really match the toy-level data.
...
### ANSWER:
The first two models adjust the line for each type of toy up or down. The first model does this by adjusting the coefficient for each toy type as a predictor, while the second one is modelling the variance in intercepts as error with some distribution. The third model also adds variance in the slopes of the lines for each toy type, which better captures the data within each type. 
...
### ANSWER:
My answer didn't meaningfully differ from the solution. 
...
### ANSWER: 
A1 (toy type random effect stdev) corresponds to the standard deviation of the distances between the toy type trend lines to the main black line. 
A2 (residual stdev) corresponds to the standard deviation of distance between each individual data point and its corresponding toy type trend line.  
A3 (intercept) corresponds to where the black line intersects the y-axis.
A4 (estimate for hrs_week) corresponds to the slope of the black line (increase in r_age for every unit increase in hrs_week)
...
### ANSWER:
No, my answer did not differ from the solution.
...
n_participants <- length(unique(efrep$PID))
# there are 30 unique participants in the data

efrep %>%
  count(PID) %>%
  filter(n == 15)
# there are no participants who have all 15 trials in the data

efrep %>% 
  count(PID) %>%
  summary()
# the average number of trials is 12. the minimum is 10. 


table(efrep$PID, efrep$audio)
# all participants have 2 or more observations for each audio type
...
ANSWER: 
The outcome of interest is the score on the SDMT. The variables we are investigating are the audio type and the interaction of headphone type and audio type. The units of observations are individual trials, which are grouped by participant. Within each cluster there is variance in the audio type, while between clusters there is variance in the headphone type.  
...
### ANSWER: 
Yes, slightly. I forgot that the headphones variable is not indexing headphone type but whether the participants listened via headphones (noise cancelling) or via speaker. 
...
efrep <- efrep %>%
  mutate(audio = relevel(factor(audio), "no_audio"), 
         headphones = relevel(factor(headphones), "S"))
...
### ANSWER:
My answer did not meaningfully differ from the solution.
...
model1 <- lme4::lmer(SDMT ~ audio * headphones + (1 + audio |  PID), 
               data = efrep)
summary(model1)
...
model_p <- lmerTest::lmer(SDMT ~ audio * headphones + (1 + audio | PID), 
                          data = efrep)
summary(model_p)
...
ggplot(data = effect(term = "audio*headphones", mod = sdmt_mod) |> as.data.frame()) +
  geom_pointrange(aes(x = audio, y = fit, 
...
In order to determine whether different types of audio interfere with executive functioning, and whether this interference differs depending upon whether or not noise-cancelling headphones are used, we used a dataset with sample scores from the SDMT obtained during trials in which participants were either not listening to audio, listening to classical music, or listening to white noise, with each participant listening either through noise-cancelling headphones or speakers in the room. Audio type was treatment-coded with "no audio" as the baseline and headphone-wearing was treatment-coded with "speaker" as the baseline. 

SDMT scores were modeled using linear mixed effects regression, with fixed effects from audio type, headphone-wearing, and the interaction between the two, and random intercepts and slopes for audio type by participant. The model was fitted with the lme4 package in R, and p values were calculated using the Satterthwaite method. Participants who did not listen to any audio and were not wearing headphones scored an average of 33.258 on the SDMT. For participants not listening through headphones, listening to music was correlated with lower scores on the SDMT (beta = -8.017, t(27.588) = -5.680, p < .001), but listening to white noise was not significantly different from not listening to any audio (beta = -0.0304, t(26.326) = -0.021, p = 0.983). When no audio was playing, listening through headphones led to significantly higher scores on the SDMT than listening through speakers (beta = 6.853, t(28.192) = 2.437, p = 0.0213). There was also an interaction between audio type and audio presentation, such that wearing headphones was associated with a significantly greater increase in SDMT scores in the white noise condition (as opposed to no audio) compared to listening through speakers (beta = 8.025, t(26.462) = 3.924, p < .001). However, this interaction was not significant when listening to music (beta = -3.587, t(27.940) = -1.793, p = 0.0839), showing that wearing headphones did not significantly affect the effect of music on executive functioning. 

Overall, the results show that listening to music negatively affects executive functioning, and this effect is not dependent on whether the music is heard through noise-cancelling headphones or speakers. Listening to white noise through speakers did not affect executive functioning (as compared to not listening to any audio), but listening to white noise through headphones did seem to boost performance on the SDMT task. Finally, wearing noise-cancelling headphones in general seems to have a positive effect on executive functioning even when not listening to any audio. 
...
### ANSWER:
I don't think my answer meaningfully differed from the solution, other than the fact that the solution included results from the optional model comparison that I did not perform. 
