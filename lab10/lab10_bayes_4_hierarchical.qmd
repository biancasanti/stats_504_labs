...
# The spread is pretty comparable across the different density levels, but the density 35 plot has the most spread while the density 10 plot is more clustered around certain values. 
...
# They do not -- if they did, it would be because smaller samples lead to higher variability since one change has a more outsized effect. In this case, the trend is the opposite, with larger samples leading to more variability. 
...
# The glm coefficient is 0.8430498, while the brm posterior mean is 0.8420535. They are very similar, which is to be expected, given that there's a good amount of data and the default prior of the brm model is only weakly informative, so the Bayesian approach should converge towards the most likely value given only the data. 
...
# The simulated survival counts with priors prior(normal(0, 1.5), class = Intercept) and prior(exponential(1), class = sd) span a pretty reasonable range, with wide coverage of different survival counts. 
...
pp_check(fit_prior_only_wider, ndraws = 100) +
  theme_bw(base_size = 14) +
  labs(title = "Prior predictive check: effect of wider prior")

# with a wider prior, there seems to be a bit more variation between different groups, and overall the prior predictive distribution fits the data less well 
...
# The model assumes that different tanks have the same underlying probability of survival, and differences between tanks result from noise and not any meaningful differences. 
...
# tank 1 had density of 10, tank 33 had density of 35
no_pool_estimates[1, c("prob_lower", "prob_upper")]
no_pool_estimates[33, c("prob_lower", "prob_upper")]


# the tank with density of 10 has a wider credible interval, because it has less data points and therefore more uncertainty in the estimate
...
# It's the standard deviation of the distribution of group-level intercepts, which represents the amount of variation between different tanks. If it were very close to 0, all the different group intercepts would have to be very similar, which would make the model very close to a complete pooling model. If it were very large, the different group intercepts would be allowed to be very different from each other and would influence each other less, making it very similar to a no pooling model. 
...
# The Rhat values are 1.00 for the sd and 1.01 for the intercept, the Bulk_ESS and Tail_ESS values are well-above 400 for both parameters, and the trace plots seem to show good mixing. 
...
# There is a very slight s-shaped pattern, such that the values below the grand mean are slightly above the diagonal (closer to the grand mean) and values above the grand mean are slightly below the diagonal (closer to the grand mean).
...
# The partial pooling model slightly lowers the estimate for this tank so that it's less than 1.0, since it's a more extreme value that's above the grand mean and it gets pulled towards the overall estimate. This is because this extreme value is likely an overestimation of the real probability due to sampling noise. 
...
# The bayesian credible intervals result from the calculation of full posterior probabilities for each group, which reflects both uncertainty in the group intercept and variance within the group. The glm estimates are based on a single estimate of the within-group variance, so the confidence intervals can be less accurate than the bayestian version.   
...
# There are differences both in the mean estimates and in the intervals of the bayesian and frequentist estimates. The Bayesian intervals tend to be wider for tanks with smaller samples and smaller for tanks with larger samples. 
...
# It captures the general shape in the data without any systematic deviations. 
...
# The partial model wins on LOO-CV, by 183.2 (with a much smaller SE of 33.1). This is not surprising, since the partial model is actually accounting for and modeling the differences between the tanks instead of assuming they are all the same.
...
# A lot of the observations are above 0.7 because we are leaving out one tank at a time, as opposed to one fish at a time, so many of the observations are influential. I would probably leave out the two observations about 1 by refitting the model with reloo(). 
