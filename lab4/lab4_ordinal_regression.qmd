...
Answer: 
The cumulative logit function computes log odds for each cumulative probability (for each category) as opposed to computing log odds for probability (normal logit link). For an outcome with $K$ categories, this produces $K - 1$ equations, one for each boundary between categories. 
