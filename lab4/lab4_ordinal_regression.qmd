...
ggplot() +
  # plot density curve for latent scores
  geom_density(data = ratings_latent, 
               aes(x = latent)) +
  # vertical dashed lines for psych thresholds
  geom_vline(xintercept = tau_psych[1], linetype = "dashed") +
  geom_vline(xintercept = tau_psych[2], linetype = "dashed") +
  geom_vline(xintercept = tau_psych[3], linetype = "dashed") +
  # add labels to each region 
  annotate("text", x = (tau_psych[1] - 0.5), y = 0.02, label = "Poor") +
  annotate("text", x = (tau_psych[2] + tau_psych[1])/2, y = 0.02, label = "Fair") +
  annotate("text", x = (tau_psych[2] + tau_psych[3])/2, y = 0.02, label = "Good") +
  annotate("text", x = (tau_psych[3] + 0.5), y = 0.02, label = "Excellent") +
  # add x axis label
  xlab("latent scores")
...
rating_freq_psych <- ratings %>%
  group_by(eval_psych) %>%
  count() %>%
  mutate(dept = "psych") %>%
  rename(category = eval_psych)

rating_freq_cs <- ratings %>%
  group_by(eval_cs) %>%
  count() %>%
  mutate(dept = "cs") %>%
  rename(category = eval_cs)

rating_freq <- bind_rows(rating_freq_psych, rating_freq_cs) %>%
  group_by(dept) %>%
  mutate(prop = n/sum(n))


ggplot(data = rating_freq) +
  geom_col(aes(x = dept, y = prop, fill = category), 
           position = "stack")

```

Answer: The only difference between the two departments is the thresholds for the ratings. This means that the thresholds control what category each latent rating will be sorted into.
...
(a) Each cumulative probability value represents the probability of getting anything under that threshold.

(b) The thresholds that would produce this are (-2.1972246, -0.4054651, 1.3862944)
...
kable(tidy(clm(eval_psych ~ 1, data = ratings)))

```

Answer: The model thresholds are very similar to the hand-calculated log-cumulative-odds, which confirms that the model is trying to estimate category boundaries in the log cumulative odds space.
...
The gray bar represents the cumulative probability of outcomes that are below the threshold between Fair and Good, P(rating \< tau_2) or P(Fair) + P(Poor).

The blue segment represents the probability of an outcome being Fair, P(tau_1 \< rating \< tau_2) or P(Fair).
...
Answer: The cumulative logit function computes log odds for each cumulative probability (for each category) as opposed to computing log odds for probability (normal logit link). For an outcome with $K$ categories, this produces $K - 1$ equations, one for each boundary between categories.
...
ratings_numeric_1 <- ratings %>%
  mutate(rating_numeric = case_when(
    eval_psych == "Poor" ~ 1, 
    eval_psych == "Fair" ~ 2, 
    eval_psych == "Good" ~ 3, 
    eval_psych == "Excellent" ~ 4
  ))

ratings_numeric_2 <- ratings %>%
  mutate(rating_numeric = case_when(
    eval_psych == "Poor" ~ 1, 
    eval_psych == "Fair" ~ 2, 
    eval_psych == "Good" ~ 3, 
    eval_psych == "Excellent" ~ 10
  ))


linear_model_1 <- lm(rating_numeric ~ effectiveness, data = ratings_numeric_1)
linear_model_2 <- lm(rating_numeric ~ effectiveness, data = ratings_numeric_2)

linear_model_1$coefficients["effectiveness"]
linear_model_2$coefficients["effectiveness"]


```

Answer: The slopes differ because the linear model is treating the numeric ratings as numeric, so different values for the excellent category lead to different fits of the ratings to the underlying real effectiveness.
...
# rank by gender plot
ggplot(data = gb) +
  geom_bar(aes(x = Gender, fill = Rank), position = "fill") +
  ylab("Proportion")
...
ggplot(data = gb %>%
  ...
  geom_bar(aes(x = age_group, fill = Rank), position = "fill")
...
Answer: the threshold estimates represent the estimates for the threshold between the three ranks in log cumulative odds space, before any additional effects from gender or age (when all predictors = 0/default values).

(b) 

```{r}
kable(tidy(m2, exponentiate = TRUE))
```

Answer: For Age, each additional year multiplies the probability of being in a higher rank by 1.0056

(c) 

```{r}
kable(predictions(m2, newdata = datagrid(Gender = "F", Age = 30)))
```

Answer: the predicted probability of rank 1 for a 30-year-old female is 34.6%.
...
Answer: given that the p-value is significant, having an interaction term is warranted here.
...
predictions(m2, 
            newdata = datagrid(Age = 25, Gender = c("F", "M"))) %>%
  kable(digits = 3, caption = "Predicted P(Rank) for 25-year-olds")

```

Answer: females have a higher predicted probability of rank 1. 
...
threshold <- m2$coefficients["3|2"]
beta_gender <- m2$coefficients["GenderM"]
beta_age <- m2$coefficients["Age"]

plogis(threshold - (beta_gender * 0 + beta_age * 25))

```

Answer: this hand-computed probability matches the one calculated using predictions.
...
Answer: Being 1 year older increases the probability of achieving rank 1 by 0.1%, which means that older people are more likely to finish first than younger people. 
...
ggplot(data = ggemmeans(m2, terms = c("Age", "Gender"))) +
  geom_line(aes(x = x, y = predicted, color = group)) +
  facet_wrap(~ response.level, 
             labeller = as_labeller(c("3" = "Rank 3", "2" = "Rank 2", "1" = "Rank 1"))) +
...
```
Answer: The proportional odds assumption is met.
...
autoplot.clm(m2, what ="qq")

```

Answer: The latent normality assumption seems to hold. 
...
sample_size <- length(gb$Gender)     # arbitrary column -- just to get number of observations

brant.test(polr(Rank ~ Gender * Age, data = gb))

kable(tidy(m3, exponentiate = TRUE, conf.int = TRUE), digits = 3)

avg_comparisons(m3, variables = "Gender", type = "prob") %>%
  kable(digits = 3, caption = "Average marginal effect of Gender on each rank")
...
We used a cumulative logit model (proportional odds) to predict technical rank (1st, 2nd, 3rd) from gender and age, including the interaction between gender and age, for a sample of N = `r sample_size` participants. A Brant test supported the proportional odds assumption (χ² = 1.295, p = 0.73). Gender was not a significant predictor of rank (OR = 3.157, 95% CI [0.848, 11.908], p = 0.088); age was also not a significant predictor of rank (OR = 1.023, 95% CI [0.999, 1.049], p = 0.064). However, the interaction of age and gender was a significant predictor of rank; each additional year of age for a male was associated with a decrease in the odds of achieving higher rank (OR = 0.962, 95% CI [0.927, 0.997], p = 0.036). In terms of predicted probabilities, switching from female to male decreased the probability of achieving rank 1 by approximately 4.3 percentage points. 
