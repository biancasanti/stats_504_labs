...
⬇️ [Download the .qmd](https://github.com/suyoghc/PSY504_Spring_2026/raw/main/posts/Bayes%202/Bayes_Lab_2_Penguins.qmd)
...
This lab is a **walkthrough** that combines Bayes (`brms`) with tidyverse tools. The focus is on the essentials of simple linear regression with `brms`.

1. Read and run through each code chunk, making sure you understand what it does before moving on.
2. Answer the conceptual questions interleaved throughout — write your responses in the empty code chunks marked `# Enter your answer here` (use R comments for text answers).
3. For questions that ask you to calculate or code, write your code in the chunks marked `# Enter your code here`.
4. At the end of each part, complete the coding exercise based on the material you've read.
:::
...
We've done from a full data set with $N = 344$ rows, to a subset with $n = 68$ rows.
("$" signs hold LaTex snippets)
...
What do the marginal histograms added by ggside tell you about the distribution of body_mass_g and bill_length_mm individually?

```{r}
# Enter your answer here

# Both variables seem to have values that are pretty well-distributed throughout a range of values, 
# although they don't seem to be necessarily normally distributed. Bill length, in particular, 
# seems to have a somewhat bimodal distribution with two different peaks.
...
$$
\text{bill\_length\_mm}_i = \beta_0 + \beta_1 \text{body\_mass\_g}_i + \epsilon_i, \quad \epsilon_i \sim \operatorname{Normal}(0, \sigma_\epsilon)
$$
...
$$
\text{bill\_length\_mm}_i \sim \operatorname{Normal}(\mu_i, \sigma), \quad \mu_i = \beta_0 + \beta_1 \text{body\_mass\_g}_i
$$
...
Can you predict what the mean value, and standard deviations of the residuals will be? Why? Calculate it. Compare this against outputs in summary(fit1.ols) and explain. Map the values you find to the latex equations before.

```{r}
# Enter your code here

# from looking at the histogram of the residuals, I would expect the mean to be around 0 and the 
# stdev to be 2 or 2.5. 

summary(fit1.ols)

# from the summary, the median (not mean) of the residuals is 0.3247 (which is close to 0, as I predicted). the standard deviation of the residuals can be approximated with the residual standard error (which is sqrt(variance/(n-2)) in this case instead of sqrt(variance/(n-1))), which is 2.887, which is also close to what I predicted. 

# these values fit with the latex equation that has 
# bill_length_mm_i = beta_0 + beta_1 * body_mass_g_i + error_i, error_i ~ Normal(0, stdev)
# so that 
# bill_length_mm_i = intercept (estimate) + body_mass_g (estimate) * body_mass_g_i + error_i, error_i ~ Normal(0, ~resid_stardard_error)
# or with values, 
# bill_length_mm_i = 32.174 + 0.00446 * body_mass_g_i + error_i, error_i ~ Normal(0, ~0.2638)
...
We'll be fitting our Bayesian models with the **brms** package.
The primary function is `brm()`. 
...
### Question 1.3 {.unnumbered}

Contrast the language of in the `brm()` output from the in the `lm()` output. Ignore 'Rhat,' 'Bulk_ESS,' and 'Tail_ESS' for now.

```{r}
# Enter your answer here

# the estimate for the intercept is pretty similar across both models (32.174 for ols, 32.18 for
# bayesian), and so is the estimate for the body_mass_g coefficient (0.0045 for ols, 0.00 for
# bayesian, although it's somewhat harder to tell the exact bayesian estimate because it's left to 
# two significant digits). the two models both have an estimate of error (std. error for ols, 
# est. error for bayesian), which has pretty similar values across both. 

# beyond estimate and error, however, the brm() output includes a 95% credible interval, 
# while the lm() output includes t value and p value as a measure of the size and uncertainty of the 
# effect. the brm() output additionally gives an estimate and credible interval for the error term, 
# while the lm() ouput includes residual standard error and then some measures of the goodness
# of fit of the model overall. 
...
bind_rows(
  tidy(fit1.ols, conf.int = TRUE) %>% select(term, estimate, conf.low, conf.high),
  tidy(fit1.b) %>% select(term, estimate, conf.low, conf.high)
...
NOTE: having issues running the above because of column selection -- commenting it out so that the code can run easily.
...
Go back to the full `penguins` data set. Make a subset of the data called `gentoo`, which is only the cases for which `species == "Gentoo"`.

-   Fit the same OLS model to these data.
-   Plot the results with `predict()`.
-   Fit the same default Bayesian `brm()` model to these data.
-   Plot the results with `conditional_effects()`.
-   Make a coefficient plot comparing the new OLS and Bayesian beta coefficients.
...
newdata <- tibble(body_mass_g = seq(from = min(gentoo$body_mass_g),
...
# plot bayesian mdoel predictions 
conditional_effects(fit2.b)

# make comparison coefficient plot
bind_rows(
  tidy(fit2.ols, conf.int = TRUE) %>% select(term, estimate, conf.low, conf.high),
  tidy(fit2.b) %>% select(term, estimate, conf.low, conf.high)
  %>% filter(term != "sd__Observation")
) %>%
  mutate(method = rep(c("lm()", "brm()"), each = 2)) %>%
...
Ignore the trace plots on the right for a moment. 
And let's consider the `pairs()` plot.
...
In the parlance of Probability, do you know what is the term by which the distributions in the diagonal of the above plot are known as? And the distributions in the off-diagonal?

```{r}
# Enter your answer here

# I'm not entirely sure, but I think the off-diagonal distributions are conditional probability distributions, while the distributions on the diagonal are unconditional probability distributions 
...
How does this concept relate to representing uncertainty? Can you anticipate how predictions are made based upon these 4000 draws and the linear regression formula?

```{r}
# Enter your answer here

# The distribution of values in the draws from the posterior allows you to quantify the uncertainty in the estimates based on the spread of the posterior draws. 
# I think that predictions for the model are calculated by taking draws for the two different beta values and calculating the expected bill length for different body masses, with one equation for each of the 4000 different draws.
...
### Question 2.3 {.unnumbered}

Discuss the skew in $\sigma$. Why it might arise, etc.?

```{r}
# Enter your answer here

# Sigma is skewed a bit to the right, such that there are more higher values. This is probably due to the fact that there is a limit on the lower end of the sigma distribution for how small the error can be, while on the higher end the error could theoretically be infinitely large and larger values likely reflect outliers 
...
### Question 2.4 {.unnumbered}

Given the skew in sigma and what you know about summary statistics, what might be the implication of using just the mean, median, or mode of posteriors to make a prediction?

```{r}
# Enter your answer here

# Using just one of the values, and especially just the mean, might not give someone a full understanding of the center of the distribution and how it's skewed. This means that it's probably best practice to visualize the distribution and try to report all three values if possible, and if not to make decisions on which value to use based on what the distribution looks like.
...
### Question 2.5 {.unnumbered}

We have done all this without yet specifying a prior. What do you think is going on?

```{r}
# Enter your answer here

# I think the brms package has a standard default prior, likely a very wide Gaussian, that it's been using as a general prior for all parameters.
...
# make plot of posterior distributions including paired values 
pairs(fit2.b, off_diag_args = list(size = 1/4, alpha = 1/4))


# save draws from the posterior
...
# make plot of posterior distribution for body mass
draws2 %>% 
  ...
  stat_halfeye(point_interval = median_qi, .width = c(.5, .99)) +
  scale_y_continuous(NULL, breaks = NULL) +
  labs(title = "Posterior distribution for Gentoo penguins",
       subtitle = "The dot marks the median.\nThe thicker line marks the 50% interval, and\nthe thinner line marks the 99% interval.",
       x = expression(hat(italic(y))*'|'*italic(x)==3733.1)) +
  coord_cartesian(xlim = c(46.5, 48.5))


# make plot of posterior distributions with all three center parameters
center_points <- draws2 %>% 
  rename(`beta[0]` = beta0,
         `beta[1]` = beta1) %>% 
  pivot_longer(cols = c(`beta[0]`, `beta[1]`, sigma), 
               names_to = "parameter") %>% 
...
# make overall spaghetti plot for all the draws
draws2 %>% 
  select(.draw, beta0, beta1) %>% 
...
# make spaghetti plot for a random subset of the draws
set.seed(888)
