...
prob_sums <- prob_table %>% 
  group_by(theta) %>%
  mutate(sum = heads_0 + heads_1 + heads_2 + heads_3 + heads_4 + heads_5)
```

Answer: this tells us that the different heads probabilities are a proper probability distribution.
...
Answer: No, it does not sum to 1, which means it's not a probability distribution.
...
Answer: It does sum to 1, which means it can be interpreted as a probability distribution.
...
```{r}
# Your code here
print(glue("There's a {posterior_table$posterior[4] * 100}% chance that theta is 0.6"))
print(glue("There's a {(posterior_table$posterior[3] + posterior_table$posterior[4]) * 100}% chance that theta is 0.5 or 0.6"))
```
...
Answer: The peak of the posterior is at 0.5995996 =\~ 0.6, which matches the MLE from Section 2.
...
Answer: The posterior mean of theta is 0.598, which is closer to 0.6. This is because with a flat prior the posterior is essentially the same as the likelihood.
...
```{r}
print(glue("There's a {mean(draws_flat$theta > 0.55 & draws_flat$theta < 0.65) * 100}% chance that theta is between 0.55 and 0.65"))
```

Answer: The original grid approximation posterior for theta = 0.6 was 30.1966%, so this is lower, which is because the probability mass is more spread out over individual values (as opposed to just 5 theta values in the grid approximation).
