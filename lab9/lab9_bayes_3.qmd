...
⬇️ [Download the .qmd](https://github.com/suyoghc/PSY504_Spring_2026/raw/main/posts/Bayes%202/Bayes_Lab_2_questions.qmd)
...
| Posterior predictive | $P(D_{new} \mid | D)$        |
...
# The model gets the general mean and spread of the distribution right, but it's not really accounting for the slight skew and other variation in the actual values. It's also not capturing how that variation might result from possible predictors that we are not modeling here. 
...
# Predict is always wider because it adds a random draw from the error term, which is a Gaussian with mean of 0 and sd of sigma. I would report fitted() when I'm trying to say something about the average predicted value for a certain set of parameters and predict() when I'm trying to say something about the predicted distribution for future data points. 
...
# Under the hood, pp_check() is doing something very similar to what we did manually -- taking a set of draws of each parameter values and using them to predict the posterior distribution from the data. Each draw corresponds to one of the light blue density plots. 
...
The dark line is your real data. The light lines are datasets the model would generate using only the priors — no learning from data at all. If these look wildly different from anything you'd expect to see, the priors are encoding unreasonable assumptions.
...
::: callout-tip
Compare the light lines to the dark line. Do they cover the same range of values? Recall the summary statistics you computed for `bill_length_mm`.
:::

```{r}
# Enter your answer here

# The predictions look pretty reasonable -- they don't really go below 0 or have unreasonably large values. If they predicted bill lengths that were too large, like 500 mm, I would have to adjust the distribution of the prior, likely by using a lower value for the standard deviation. 
...
# The results are very similar, with some small differences in the values for the intercept, which means the manually set priors match the default priors quite well. 
...
# It means that body_mass_g, which is a predictor in the fit1.b model but not in the fit0.b model, is a useful predictor of bill length that captures variation in the data. 
...
posterior_summary(fit_g.b)
b0    <- posterior_summary(fit_g.b)[1, 1]
b1    <- posterior_summary(fit_g.b)[2, 1]
sigma <- posterior_summary(fit_g.b)[3, 1]

# plot posterior distributions with data points
crossing(body_mass_g    = seq(from = 3900, to = 6500, length.out = 200),
         bill_length_mm = seq(from = 35, to = 60, length.out = 200))  %>% 
  mutate(density = dnorm(x = bill_length_mm, 
                         mean = b0 + b1 * body_mass_g,
                         sd = sigma)) %>% 
...
# plot of model posterior distributions
as_draws_df(fit_g.b) %>% 
  rename(`beta[0]` = b_Intercept,
         `beta[1]` = b_body_mass_g) %>% 
  pivot_longer(cols = c(`beta[0]`, `beta[1]`, sigma), 
  ...
  stat_histinterval(.width = .95, normalize = "panels") +
  scale_y_continuous(NULL, breaks = NULL) +
  labs(title = "Gentoo penguin model",
       subtitle = "95% credible intervals",
       x = "parameter space") +
  facet_wrap(~ parameter, scales = "free", labeller = label_parsed)


# create new data df for gentoo penguins
nd_g <- tibble(body_mass_g = seq(
  from = min(gentoo$body_mass_g),
  to = max(gentoo$body_mass_g),
  length.out = 100))

# plot of conditional mean distribution
fitted(fit_g.b, 
       newdata = nd_g,
       summary = F) %>% 
  data.frame() %>% 
  set_names(pull(nd_g, body_mass_g)) %>% 
  mutate(draw = 1:n()) %>% 
  pivot_longer(-draw) %>% 
  mutate(body_mass_g = as.double(name)) %>%
